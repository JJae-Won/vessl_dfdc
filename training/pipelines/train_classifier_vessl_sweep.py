...
def main():
    # wandb.init(project="jw-vessl-test", reinit=True)
    vessl.init()
    ...
    bs = int(os.environ.get('batch_size', 12))
    opt_type = str(os.environ.get('type', "SGD"))
    weight_decay = float(os.environ.get('weight_decay', 1e-4))
    learning_rate = float(os.environ.get('learning_rate', 0.01))
    
    conf['optimizer']['batch_size'] = bs
    conf['optimizer']['type'] = opt_type
    conf['optimizer']['weight_decay'] = weight_decay
    conf['optimizer']['learning_rate'] = learning_rate
    ...
    data_val = DeepFakeClassifierDataset(mode="val",
                                         ...
                                         normalize=conf.get("normalize", None))
    val_data_loader = DataLoader(data_train, batch_size=batch_size * 2, num_workers=args.workers, shuffle=False,
                                 pin_memory=False)
...
def validate(net, data_loader, prefix=""):
    ...
    # print(y[fake_idx], x[fake_idx])
    # print(y[fake_idx].shape, x[fake_idx].shape)
    # print(y[real_idx], x[real_idx])
    # print(y[real_idx].shape, x[real_idx].shape)

    fake_loss = log_loss(y[fake_idx].round().astype(np.int8), x[fake_idx], labels=[0, 1])
    real_loss = log_loss(y[real_idx].round().astype(np.int8), x[real_idx], labels=[0, 1])
    ...
    # wandb.log({
    #     "val/fake_loss" : fake_loss,
    #     "val/real_loss" : real_loss,
    #     "val/bce" : bce
    # })
...
def train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
    ...
    print("train/fake_loss", fake_losses.avg)
    print("train/real_loss", real_losses.avg)
    vessl.log(
        step=current_epoch,
        payload={
            "train/loss": losses.avg,
            "train/fake_loss": fake_losses.avg, 
            "train/real_loss": real_losses.avg
        }
    )
    # wandb.log({
    #         "train/loss": losses.avg,
